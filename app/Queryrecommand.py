...
Qlist = [
    ...
    QueryInfo(
        ...
        "중", "Address"),
    QueryInfo("SELECT DISTINCT Salary AS SecondHighestSalary FROM Employee ORDER BY Salary DESC LIMIT 1 OFFSET 1", "중",
              "Employee"),
    QueryInfo(
    ...
    QueryInfo(
        "SELECT customers.name AS 'Customers' FROM customer WHERE customer.id NOT IN (SELECT customerid FROM orders)",
        "상", "Customer"),
    QueryInfo("DELETE p1 FROM Person p1, Person p2 WHERE p1.Email = p2.Email AND p1.ID > p2.ID", "하", "Person"),
    QueryInfo(
        "SELECT weather.id AS 'ID' FROM weather JOIN weather w ON DATEDIFF(weather.recordDate, w.recodDate)=1 AND wather.Temprature > w.Temprature",
        "상", "Weather"),
    QueryInfo(
    ...
    QueryInfo(
        "SELECT classnum, tel, studentnum , rank() over(order by classnum DESC) as ranking FROM table_school ORDER BY tel",
        "상", "school"),
...
class AboutOurQuery:
    ...
    def Similar(self):
        Qgram = ['SELECT', 'AS', 'FROM', 'WHERE', 'ORDER', 'BY', 'DISTINCT', 'MAX', 'MIN', '*', 'BETWEEN', 'AND',
                 'DESC', '>', '<', 'LEFT', 'RIGHT', 'JOIN', 'DELETE', 'LIMIT', 'OFFSET', 'DATEDIFF', 'OR', 'NOT', 'IN',
                 'LIKE']
        txt = ''
        ComLabel = []
        Docs = []
